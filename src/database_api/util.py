...
from src.util.db_util import Conwrapper, create_value_string, sanitize, create_entry_string
...
def sql_assemble_modifiers(primary_key=False, auto_increment=False) -> Union[str, None]:
    modifiers = []
    if primary_key:
...
MappedQueryResult = List[Dict[str, Any]]
LookupQueryResult = Dict[Any, Dict[str, Any]]
LookupGroupsQueryResult = Dict[Any, List[Dict[str, Any]]]


def _convert_map_to_lookup_groups(formatted: List[Dict[str, Any]], key: str, drop_key=False) -> LookupGroupsQueryResult:
    return collection_util.group_dicts_on_key(formatted, key)


def _convert_map_to_lookup(formatted: List[Dict[str, Any]], key: str) -> LookupQueryResult:
...
class BaseClient:
    ...
    def _fetch_all(self, query: str) -> QueryResult:
        with Conwrapper(self.db_path) as (con, cursor):
            cursor.execute(query)
            return cursor.fetchall()

    def _fetch_all_mapped(self, query: str, mapping: Union[List, Tuple]) -> MappedQueryResult:
        rows = self._fetch_all(query)
        return collection_util.tuple_to_dict(rows, mapping)

    def _fetch_all_lookup(self, query: str, mapping: Union[List, Tuple], key: str) -> LookupQueryResult:
        formatted = self._fetch_all_mapped(query, mapping)
        return _convert_map_to_lookup(formatted, key)

    def _fetch_all_lookup_groups(self, query: str, mapping: Union[List, Tuple], key: str) -> LookupGroupsQueryResult:
        formatted = self._fetch_all_mapped(query, mapping)
        return _convert_map_to_lookup_groups(formatted, key)
