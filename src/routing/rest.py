...
from src.database.clients import FileClient, PageClient, TagClient
...
def rest_tag_get(request, tag_id):
    ...
    results = client.fetch(ids=[int(tag_id)])
    if len(results) != 1:
        return None, 404
...
def rest_file_get(request, img_id):
    client = FileClient(db_path=database_path)
    request_format = __get_request_format(request)
    results = client.fetch(ids=[int(img_id)])
    if len(results) != 1:
        return None, 404
