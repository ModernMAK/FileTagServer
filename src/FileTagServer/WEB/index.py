...
def add_routes(app: WebsiteApp):
    ...
    def root_folder():
        ...
        ancestry = build_ancestry(app.database, add_self=False)  # Builds root only
        files = None  # get_orphaned_files()
        tags = None
        context = build_context(app.webconv, "Root", None, ancestry, folders, files, tags)
    ...
    def sub_folder(folder_id: int):
        ...
        ancestry = build_ancestry(app.database, folder.name, reformat(folder_route, folder_id=folder_id), folder.path)
        context = build_context(app.webconv, folder.name, folder.description, ancestry, subfolders, files, tags, tag_lookup)
        return _folder(context)
    ...
    def orphaned_file_folder():
        ...
        ancestry = build_ancestry(app.database, "Orphaned Files", orphaned_files_route)
        context = build_context(app.webconv, "Orphaned Files", "Files with no parent folder.", ancestry, subfolders, files, tags, tag_lookup)
        return _folder(context)

    @app.get(file_route)
    def file(file_id: int, range: Optional[str] = Header(None)):
        return file_preview(file_id, range)
